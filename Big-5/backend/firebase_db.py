...
class FirestoreDB:
    client = db  # Expose the client for main.py

    @staticmethod
    def save_assessment(user_data, assessment_results, raw_answers):
        if FirestoreDB.client is None:
        ...
        try:
            ...
            update_time, doc_ref = FirestoreDB.client.collection('assessments').add(doc_data)
            return doc_ref.id
        except Exception as e:
    ...
    def save_team(team_name, member_ids, team_analysis):
        ...
        try:
            ...
            update_time, doc_ref = FirestoreDB.client.collection('teams').add(doc_data)
            return doc_ref.id
        except Exception as e:
    ...
    def get_user_profile_id(doc_id):
        if FirestoreDB.client is None: return 0
        try:
            doc = FirestoreDB.client.collection('assessments').document(doc_id).get()
